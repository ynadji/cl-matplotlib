"""polar-scatter.py — Reference for examples/polar-scatter.lisp"""
import matplotlib
matplotlib.use('Agg')
...
# 20 evenly spaced points (not inclusive of 2π)
...
# CL polar-affine transform, markers only (linewidth=0)
x_data = r * np.cos(theta) * 0.5 / r_max + 0.5
y_data = r * np.sin(theta) * 0.5 / r_max + 0.5
ax.plot(x_data, y_data, color='C3', linewidth=0, marker='o', markersize=6, zorder=2)

title_pad_px = 6.0 * (100.0 / 72.0) + 1.0
y_title = 1.0 + title_pad_px / DH
ax.text(0.5, y_title, 'Polar Scatter',
