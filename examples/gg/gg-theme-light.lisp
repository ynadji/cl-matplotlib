;;;; gg-theme-light.lisp — twin of reference_scripts/gg/gg-theme-light.py
...
(let ((data (list :x (coerce (loop for i from 0 below 21 collect (* i 0.5d0)) 'vector)
                  :y (coerce (loop for i from 0 below 21
                             ...
                             'vector)))
      (theme (gg:theme-light)))
  (gg:ggsave (gg:stack (gg:ggplot data (gg:aes :x :x :y :y))
               (gg:geom-point)
               (gg:geom-line)
               theme)
             "examples/gg/gg-theme-light.png")
  (format t "~&Saved examples/gg/gg-theme-light.png~%"))
