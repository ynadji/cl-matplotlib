...
;;; ============================================================
;;; PDF Hatch pattern rendering
;;; ============================================================

(defun %render-hatch-pdf (path transform gc renderer)
  "Render hatch pattern over a filled path area in PDF.
Clips to PATH shape, then tiles the hatch pattern across the bounding box.
Uses the same hatch-get-path infrastructure as the Vecto backend."
  (let* ((hatch-str (mpl.rendering:gc-hatch gc))
    ...
    (when hatch-path
      (let* ((dpi (renderer-dpi renderer))
        ...
        (when (> min-x max-x) (return-from %render-hatch-pdf nil))
        ;; Use saved state for clipping + hatch stroke
            ...
            (loop for txi from tx0 below tx1
                  do (loop for tyi from ty0 below ty1
                           do (let* ((ox (float (* txi tile-size) 1.0d0))
                                ...
                                (%trace-path-to-pdf hatch-path tile-xf))))
            (pdf:stroke)))))))
  ...
  (let ((edge-color (%gc-edge-color gc))
    ...
    (let ((hatch (mpl.rendering:gc-hatch gc)))
      (when (and hatch (stringp hatch) (not (string= hatch "")) face-color)
        (%render-hatch-pdf path transform gc renderer)))))
