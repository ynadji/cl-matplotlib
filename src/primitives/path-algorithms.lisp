...
Returns T if inside, NIL if outside."
  ...
  (let ((n (length vertices))
    ...
    ;; Coerce to vector for O(1) random access (j = i-1 pattern)
    (let* ((verts-vec (coerce vertices 'vector))
           (j (1- n)))
      (dotimes (i n)
        (let ((xi (car (aref verts-vec i)))
              (yi (cdr (aref verts-vec i)))
              (xj (car (aref verts-vec j)))
              (yj (cdr (aref verts-vec j))))
