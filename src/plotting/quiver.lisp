         ...
         (let ((x-flat (if (and (listp x) (listp (first x)))
           ...
           (if (and (< (length x-flat) (length u-flat))
               ...
               (let* ((nx (length x-flat))
                      (ny (length y-flat))
                      ;; Coerce to vectors for O(1) indexed access in nested loops
                      (x-vec (coerce x-flat 'vector))
                      (y-vec (coerce y-flat 'vector))
                      (x-grid nil)
                      (y-grid nil))
                 (dotimes (j ny)
                   (let ((yj (float (aref y-vec j) 1.0d0)))
                     (dotimes (i nx)
                       (push (float (aref x-vec i) 1.0d0) x-grid)
                       (push yj y-grid))))
         ...
         (if is-2d
             ...
             (let* ((nrows (length u))
               ...
               (loop for u-row in u
                     for v-row in v
                     for j from 0
                     do (let ((jf (float j 1.0d0))
                              (u-row-vec (coerce u-row 'vector))
                              (v-row-vec (coerce v-row 'vector)))
                          (dotimes (i ncols)
                            (push (float i 1.0d0) x-list)
                            (push jf y-list)
                            (push (float (aref u-row-vec i) 1.0d0) u-flat)
                            (push (float (aref v-row-vec i) 1.0d0) v-flat))))
