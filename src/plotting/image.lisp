...
Returns the created AxesImage."
  ...
  (let* ((cmap-obj (cond
    ...
    ;; For origin='upper' (default) with the DEFAULT extent, invert the y-axis
    ;; so that row 0 is at the top of the image and y=0 maps to the top —
    ;; matching matplotlib's behavior. This ensures text annotations placed at
    ;; y=i align with data[i,*] pixels. When the user supplies an explicit
    ;; :extent, it already encodes the orientation and is used as-is.
    (when (and (eq origin :upper) (null extent))
